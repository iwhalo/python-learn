...
# @Author  : fuqingyun
# @File    : performanceTest_byLocust1.py
# @Description : @task装饰器设置任务权重
import os
...
# 3.根据自身业务定义测试任务类 &&  6.任务标记与权重
'''
前面提到的@task装饰器除了可以声明测试任务外，还可以设置任务权重
...
你可以认为10个用户有5个用户去执行了task1任务，3个用户去执行了task2任务，2个用户去执行了task3任务。
'''
